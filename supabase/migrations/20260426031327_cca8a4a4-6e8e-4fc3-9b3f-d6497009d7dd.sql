...
DECLARE
  field_name TEXT; field_value TEXT; allowed TEXT[];
BEGIN
...
CREATE TABLE public.users (
  ...
  role TEXT NOT NULL DEFAULT 'agent', avatar_url TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE public.tags (
  ...
  is_custom_color BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE public.quick_replies (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
  shortcut TEXT NOT NULL, content TEXT NOT NULL, media_url TEXT,
...
CREATE TABLE public.libraries (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
  name TEXT NOT NULL, media_type TEXT NOT NULL, media_url TEXT NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE public.business_hours (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
  day_of_week TEXT NOT NULL, open_at TIME, close_at TIME,
...
CREATE TABLE public.automation_flows (
  ...
  name TEXT NOT NULL, shortcut TEXT,
...
CREATE TRIGGER validate_automation_flows_channel BEFORE INSERT OR UPDATE ON public.automation_flows
FOR EACH ROW EXECUTE FUNCTION public.validate_enum_field('channel_type', '{whatsapp,whatsapp_official,instagram,email,messenger,widget}');
...
-- CHANNELS (multicanal, substitui whatsapp_instances)
CREATE TABLE public.channels (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
  channel_type TEXT NOT NULL DEFAULT 'whatsapp',
  display_name TEXT NOT NULL,
  phone_number TEXT, token TEXT,
  auth_type TEXT DEFAULT 'qr_code',
  ...
  qr_code TEXT,
  welcome_flow_id UUID REFERENCES public.automation_flows(id) ON DELETE SET NULL,
  default_response_flow_id UUID REFERENCES public.automation_flows(id) ON DELETE SET NULL,
  closed_flow_id UUID REFERENCES public.automation_flows(id) ON DELETE SET NULL,
  default_response_delay_value INT NOT NULL DEFAULT 24,
  default_response_delay_unit TEXT NOT NULL DEFAULT 'hours',
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
ALTER TABLE public.channels ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow all access to channels" ON public.channels FOR ALL USING (true) WITH CHECK (true);
CREATE TRIGGER validate_channels_type BEFORE INSERT OR UPDATE ON public.channels
FOR EACH ROW EXECUTE FUNCTION public.validate_enum_field('channel_type', '{whatsapp,whatsapp_official,instagram,email,messenger,widget}');
CREATE TRIGGER validate_channels_status BEFORE INSERT OR UPDATE ON public.channels
FOR EACH ROW EXECUTE FUNCTION public.validate_enum_field('status', '{connected,disconnected,pending,error}');
CREATE TRIGGER validate_channels_delay_unit BEFORE INSERT OR UPDATE ON public.channels
FOR EACH ROW EXECUTE FUNCTION public.validate_enum_field('default_response_delay_unit', '{minutes,hours,days}');
...
CREATE TABLE public.messages (
  ...
  direction TEXT NOT NULL, type TEXT NOT NULL DEFAULT 'text',
...
CREATE TABLE public.group_manager_groups (
  ...
  tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
  flow_id UUID REFERENCES public.automation_flows(id) ON DELETE SET NULL,
  name TEXT NOT NULL, whatsapp_group_id TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- NODE DETAIL TABLES
CREATE TABLE public.node_content_items (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  text_strikethrough BOOLEAN DEFAULT FALSE,
  interval_seconds INT,
  media_url TEXT, media_filename TEXT,
  send_as_recorded BOOLEAN DEFAULT FALSE,
  contact_name TEXT, contact_phone TEXT
);
ALTER TABLE public.node_content_items ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow all access to node_content_items" ON public.node_content_items FOR ALL USING (true) WITH CHECK (true);
CREATE TRIGGER validate_node_content_items_type BEFORE INSERT OR UPDATE ON public.node_content_items
FOR EACH ROW EXECUTE FUNCTION public.validate_enum_field('item_type', '{text,interval,image,audio,video,file,contact,sticker}');

CREATE TABLE public.node_menu_options (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  node_id UUID NOT NULL REFERENCES public.flow_nodes(id) ON DELETE CASCADE,
  position INT NOT NULL DEFAULT 0,
  label TEXT NOT NULL, keyword TEXT
);
ALTER TABLE public.node_menu_options ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow all access to node_menu_options" ON public.node_menu_options FOR ALL USING (true) WITH CHECK (true);

CREATE TABLE public.node_save_configs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  node_id UUID NOT NULL REFERENCES public.flow_nodes(id) ON DELETE CASCADE,
  wait_time_value INT NOT NULL DEFAULT 1,
  wait_time_unit TEXT NOT NULL DEFAULT 'hours',
  message_before_wait TEXT,
  save_field_name TEXT,
  accept_media_as_response BOOLEAN NOT NULL DEFAULT FALSE,
  max_retry_attempts INT DEFAULT NULL,
  UNIQUE (node_id)
);
ALTER TABLE public.node_save_configs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow all access to node_save_configs" ON public.node_save_configs FOR ALL USING (true) WITH CHECK (true);
CREATE TRIGGER validate_node_save_configs_unit BEFORE INSERT OR UPDATE ON public.node_save_configs
FOR EACH ROW EXECUTE FUNCTION public.validate_enum_field('wait_time_unit', '{minutes,hours,days}');

CREATE TABLE public.node_action_configs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  node_id UUID NOT NULL REFERENCES public.flow_nodes(id) ON DELETE CASCADE,
  position INT NOT NULL DEFAULT 0,
  action_type TEXT NOT NULL,
  tag_id UUID REFERENCES public.tags(id) ON DELETE SET NULL,
  target_flow_id UUID REFERENCES public.automation_flows(id) ON DELETE SET NULL,
  variable_name TEXT, variable_value TEXT,
  http_url TEXT, http_method TEXT, http_body JSONB
);
ALTER TABLE public.node_action_configs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow all access to node_action_configs" ON public.node_action_configs FOR ALL USING (true) WITH CHECK (true);
CREATE TRIGGER validate_node_action_configs_type BEFORE INSERT OR UPDATE ON public.node_action_configs
FOR EACH ROW EXECUTE FUNCTION public.validate_enum_field('action_type', '{add_tag,remove_tag,remove_all_tags,flow_connection,set_variable,assign_department,assign_agent,remarketing,http_request}');

CREATE TABLE public.node_randomizer_options (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  node_id UUID NOT NULL REFERENCES public.flow_nodes(id) ON DELETE CASCADE,
  position INT NOT NULL DEFAULT 0,
  percentage INT NOT NULL DEFAULT 50
);
ALTER TABLE public.node_randomizer_options ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow all access to node_randomizer_options" ON public.node_randomizer_options FOR ALL USING (true) WITH CHECK (true);

-- SEED default tenant
INSERT INTO public.tenants (id, company_name, subdomain) 
VALUES ('00000000-0000-0000-0000-000000000001', 'Zap Estratégico', 'default');
...
CREATE INDEX idx_node_content_node ON public.node_content_items (node_id, position);
CREATE INDEX idx_node_menu_node ON public.node_menu_options (node_id, position);
CREATE INDEX idx_node_action_node ON public.node_action_configs (node_id, position);
CREATE INDEX idx_node_randomizer_node ON public.node_randomizer_options (node_id, position);